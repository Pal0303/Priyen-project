...
from .models import Candidate, Company, Query
...
class Details(generic.ListView):
    ...
    def get_queryset(self):
        ...
        for _ in os.listdir("/Users/priyen/Desktop/AI-based-job-application-portal-master/mysite/media/jobs/pdfs"):
            resume = _
            break;
...
def temp(request):
    return render(request,'jobs/temp.html')
def company(request):
    return render(request,'jobs/company.html')
def jobsearch(request):
    return render(request,'jobs/jobsearch.html')
def interview(request):
      # You can add context data here if needed
    return render(request, 'jobs/interview.html')
def jobsearch(request):
